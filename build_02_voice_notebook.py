...
def create_generation_notebook():
    ...
    cells.append({
        ...
        "metadata": {},
        "source": [
            "# Phase 2: LLM Artifact Generation & Structured Voice Packs\n",
            "\n",
            "This notebook runs Gemma to generate 15 educational artifacts, including specifically targeted `faqs` and `common_doubts`.\n",
            "It also includes a Phase 3 that runs the **Svara TTS ONNX** model to generate a structured directory of `.wav` audio files (concept audio, summaries, full chapter narration, flashcards with pauses, etc.) directly mapped to the generated artifacts.\n"
        ]
    })
    ...
    cells.append({
        ...
        "source": [
            ...
            "\n",
            "for path in [CACHE_ROOT, PACK_ROOT, VOICE_ROOT]:\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "This section parses the generated JSON artifacts to synthesize high-value voice files (flashcards, summaries, etc.) using Svara TTS."
        ]
    })
    ...
    cells.append({
        ...
        "source": [
            ...
            "}}\n",
            "\n",
            "async function synthesizeWav(model, tokenizer, decoder, textChunks, outputPath, insertPauseBetween = false) {{\n",
            "  if (fs.existsSync(outputPath)) return outputPath;\n",
            "  console.log(`Synthesizing: ${{outputPath}}`);\n",
            "  let combinedSamples = [];\n",
            ...
            "      }}\n",
            "    }} catch (e) {{ console.error('Failed chunk:', e.message); }}\n",
            "  }}\n",
            ...
            "    manifest.chapter_id = s.chapter_id;\n",
            "    let text = s.payload.summary;\n",
            "    if (text) {{\n",
            ...
            "  let objText = objectives.map(o => (o.payload.items || []).map(i => i.objective).join('. ')).join('. ');\n",
            "  if (objText.trim()) {{\n",
            ...
            "    for (let item of s.payload.items || []) {{\n",
            ...
            "      if(await synthesizeWav(model, tokenizer, decoder, parts, path.join(VOICE_DIR, p), true))\n",
            ...
            "      }}\n",
            "  }};\n",
            "  if (fs.existsSync(structuredChaptersPath)) recurseDirs(structuredChaptersPath);\n",
            "  if (fullText.trim()) {{\n",
            "     let p = 'full_chapter.wav';\n",
            "     if(await synthesizeWav(model, tokenizer, decoder, chunkText(fullText, 30), path.join(VOICE_DIR, p)))\n",
            "         manifest.full_narration = p;\n",
            "  }}\n",
            "\n",
            "  // Write manifest\n",
            "  fs.writeFileSync(path.join(VOICE_DIR, 'voice_manifest.json'), JSON.stringify(manifest, null, 2));\n",
            "  console.log('Voice manifest generated!');\n",
            "}}\n",
